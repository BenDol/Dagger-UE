...
void UElimChainProcessor::StartListening()
{
    ...
    AddListenerHandle(MessageSubsystem.RegisterListener(ElimChain::TAG_Dagger_Elimination_Message, this, &ThisClass::OnEliminationMessage));
}

void UElimChainProcessor::OnEliminationMessage(FGameplayTag Channel, const FDaggerVerbMessage& Payload)
{
    ...
    if (Payload.Instigator != Payload.Target)
    {
		if (APlayerState* InstigatorPS = Cast<APlayerState>(Payload.Instigator))
		{
			...
			else
			{
				...
				{
					FDaggerVerbMessage ElimChainMessage;
