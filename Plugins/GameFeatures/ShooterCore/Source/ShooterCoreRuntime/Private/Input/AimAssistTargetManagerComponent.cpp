...
#include "Character/DaggerHealthComponent.h"
...
const FDaggerAimAssistTarget* FindTarget(const TArray<FDaggerAimAssistTarget>& Targets, const UShapeComponent* TargetComponent)
{
    const FDaggerAimAssistTarget* FoundTarget = Targets.FindByPredicate(
    [&TargetComponent](const FDaggerAimAssistTarget& Target)
    {
...
void UAimAssistTargetManagerComponent::GetVisibleTargets(const FAimAssistFilter& Filter, const FAimAssistSettings& Settings, const FAimAssistOwnerViewData& OwnerData, const TArray<FDaggerAimAssistTarget>& OldTargets, OUT TArray<FDaggerAimAssistTarget>& OutNewTargets)
{
    ...
    // Do a world trace on the Aim Assist channel to get any visible targets
    {
		...
		if(DaggerConsoleVariables::bDrawDebugViewfinder)
		{
    ...
    // Gather targets that are in front of the player
    {
		...
		for (FAimAssistTargetOptions& AimAssistTarget : NewTargetData)
		{
			...
			const FDaggerAimAssistTarget* OldTarget = FindTarget(OldTargets, AimAssistTarget.TargetShapeComponent.Get());
			...
			FDaggerAimAssistTarget NewTarget;
    ...
    {
		OutNewTargets.Sort([](const FDaggerAimAssistTarget& TargetA, const FDaggerAimAssistTarget& TargetB)
		{
    ...
    {
		for (FDaggerAimAssistTarget& Target : OutNewTargets)
		{
...
bool UAimAssistTargetManagerComponent::DoesTargetPassFilter(const FAimAssistOwnerViewData& OwnerData, const FAimAssistFilter& Filter, const FAimAssistTargetOptions& Target, const float AcceptableRange) const
{
    ...
    if (const ACharacter* TargetCharacter = Cast<ACharacter>(TargetOwningActor))
    {
		...
		{
			if (const ADaggerPlayerState* PS = TargetCharacter->GetPlayerState<ADaggerPlayerState>())
			{
		...
		{
			if (const UDaggerHealthComponent* HealthComponent = UDaggerHealthComponent::FindHealthComponent(TargetCharacter))
			{
...
void UAimAssistTargetManagerComponent::DetermineTargetVisibility(FDaggerAimAssistTarget& Target, const FAimAssistSettings& Settings, const FAimAssistFilter& Filter, const FAimAssistOwnerViewData& OwnerData)
{
    UWorld* World = GetWorld();
    check(World);
