...
#include "Input/DaggerAimSensitivityData.h"
#include "Player/DaggerLocalPlayer.h"
#include "Player/DaggerPlayerState.h"
#include "SceneView.h"
#include "Settings/DaggerSettingsShared.h"
...
namespace DaggerConsoleVariables
{
...
///////////////////////////////////////////////////////////////////
// FDaggerAimAssistTarget

void FDaggerAimAssistTarget::ResetTarget()
{
...
FRotator FDaggerAimAssistTarget::GetRotationFromMovement(const FAimAssistOwnerViewData& OwnerInfo) const
{
...
float FDaggerAimAssistTarget::CalculateRotationToTarget2D(float TargetX, float TargetY, float OffsetY) const
{
    if (TargetX <= 0.0f)
    {
		return 0.0f;
...
void FAimAssistOwnerViewData::UpdateViewData(const APlayerController* PC)
{
    ...
    // Set the Team ID
    if (ADaggerPlayerState* DaggerPS = PlayerController->GetPlayerState<ADaggerPlayerState>())
    {
		TeamID = DaggerPS->GetTeamId();
    }
    else
    {
		TeamID = INDEX_NONE;
...
FInputActionValue UAimAssistInputModifier::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
...
#if ENABLE_DRAW_DEBUG
    if (DaggerConsoleVariables::bDrawAimAssistDebug)
    {
    ...
    if (!DaggerConsoleVariables::bEnableAimAssist)
    {
		return CurrentValue;
    }
...
void UAimAssistInputModifier::UpdateTargetData(float DeltaTime)
{
    ...
    SwapTargetCaches();
    const TArray<FDaggerAimAssistTarget>& OldTargetCache = GetPreviousTargetCache();
    TArray<FDaggerAimAssistTarget>& NewTargetCache = GetCurrentTargetCache();
    ...
    for (FDaggerAimAssistTarget& Target : NewTargetCache)
    {
...
const float UAimAssistInputModifier::GetSensitivityScalar(const UDaggerSettingsShared* SharedSettings) const
{
    if (SharedSettings && SensitivityLevelTable)
    {
		const EDaggerGamepadSensitivity Sens = TargetingType == EDaggerTargetingType::Normal ? SharedSettings->GetGamepadLookSensitivityPreset() : SharedSettings->GetGamepadTargetingSensitivityPreset();
		return SensitivityLevelTable->SensitivityEnumToFloat(Sens);
    }
    
    UE_LOG(LogAimAssist, Warning, TEXT("SensitivityLevelTable is null, using default value!"));
    return (TargetingType == EDaggerTargetingType::Normal) ? 1.0f : 0.5f;    
}
...
FRotator UAimAssistInputModifier::UpdateRotationalVelocity(APlayerController* PC, float DeltaTime, FVector CurrentLookInputValue, FVector CurrentMoveInputValue)
{
    ...
    const TArray<FDaggerAimAssistTarget>& TargetCache = GetCurrentTargetCache();

    float LookStickDeadzone = 0.25f;
    float MoveStickDeadzone = 0.25f;
    float SettingStrengthScalar = (TargetingType == EDaggerTargetingType::Normal) ? 1.0f : 0.5f;

    if (UDaggerLocalPlayer* LP = Cast<UDaggerLocalPlayer>(PC->GetLocalPlayer()))
    {
		UDaggerSettingsShared* SharedSettings = LP->GetSharedSettings();
		LookStickDeadzone = SharedSettings->GetGamepadLookStickDeadZone();
		MoveStickDeadzone = SharedSettings->GetGamepadMoveStickDeadZone();
		SettingStrengthScalar = GetSensitivityScalar(SharedSettings);
    }
    
    for (const FDaggerAimAssistTarget& Target : TargetCache)
    {
		if (Target.bUnderAssistOuterReticle && Target.bIsVisible)
		{
...
void UAimAssistInputModifier::CalculateTargetStrengths(const FDaggerAimAssistTarget& Target, float& OutPullStrength, float& OutSlowStrength) const
{
    const bool bIsADS = (TargetingType == EDaggerTargetingType::ADS);
...
{
    if (!Canvas || !OwnerViewData.IsDataValid() || !DaggerConsoleVariables::bDrawAimAssistDebug)
    {
		return;
    }
