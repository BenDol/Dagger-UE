...
#include "Input/DaggerInputModifiers.h"
...
class UDaggerAimSensitivityData;
class UDaggerSettingsShared;
...
struct FAimAssistOwnerViewData
{
    ...
    /** The ID of the team that this owner is from. It is populated from the ADaggerPlayerState. If the owner does not have a player state, then it will be INDEX_NONE */
    int32 TeamID = INDEX_NONE;
};

/** A container for keeping the state of targets between frames that can be cached */
USTRUCT(BlueprintType)
struct FDaggerAimAssistTarget
{
    GENERATED_BODY()

    FDaggerAimAssistTarget() { ResetTarget(); }
...
public:
    ...
    UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
    EDaggerTargetingType TargetingType = EDaggerTargetingType::Normal;

    /** Asset that gives us access to the float scalar value being used for sensitivty */
    UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
    TObjectPtr<const UDaggerAimSensitivityData> SensitivityLevelTable = nullptr;
...
protected:
    ...
    */
    ...
    void CalculateTargetStrengths(const FDaggerAimAssistTarget& Target, float& OutPullStrength, float& OutSlowStrength) const;

    FRotator GetLookRates(const FVector& LookInput);
    
    void SwapTargetCaches() { TargetCacheIndex ^= 1; }
    const TArray<FDaggerAimAssistTarget>& GetPreviousTargetCache() const    { return ((TargetCacheIndex == 0) ? TargetCache1 : TargetCache0); }
    TArray<FDaggerAimAssistTarget>& GetPreviousTargetCache()				{ return ((TargetCacheIndex == 0) ? TargetCache1 : TargetCache0); }

    const TArray<FDaggerAimAssistTarget>& GetCurrentTargetCache() const    { return ((TargetCacheIndex == 0) ? TargetCache0 : TargetCache1); }
    TArray<FDaggerAimAssistTarget>& GetCurrentTargetCache()				{ return ((TargetCacheIndex == 0) ? TargetCache0 : TargetCache1); }

    bool HasAnyCurrentTargets() const { return !GetCurrentTargetCache().IsEmpty(); }

    const float GetSensitivityScalar(const UDaggerSettingsShared* SharedSettings) const;
    
    // Tracking of the current and previous frame's targets
    UPROPERTY()
    TArray<FDaggerAimAssistTarget> TargetCache0;

    UPROPERTY()
    TArray<FDaggerAimAssistTarget> TargetCache1;
