...
#include "AbilitySystem/Attributes/DaggerHealthSet.h"
...
#include "System/DaggerAssetManager.h"
...
TEST_CLASS_WITH_FLAGS(AbilitySpawnerMapTest, "Project.Functional Tests.ShooterTests.GameplayAbility", EAutomationTestFlags::EditorContext | EAutomationTestFlags::ProductFilter)
{
	...
	const UDaggerHealthSet* HealthSet{ nullptr };
	...
	{
		const TSubclassOf<UGameplayEffect> DamageEffect = UDaggerAssetManager::GetSubclass(UDaggerGameDataBase::Get().DamageGameplayEffect_SetByCaller);
		...
		SpecHandle.Data->SetSetByCallerMagnitude(CommonGameplayTags::SetByCaller_Damage, Damage);
	...
	BEFORE_EACH()
	{
		...
		TestCommandBuilder
			...
			.Do([this]() {
				Player = CastChecked<ADaggerCharacter>(Spawner->FindFirstPlayerPawn());
				AbilitySystemComponent = Player->GetDaggerAbilitySystemComponent();
				ASSERT_THAT(IsNotNull(AbilitySystemComponent));

				HealthSet = AbilitySystemComponent->GetSetChecked<UDaggerHealthSet>();
