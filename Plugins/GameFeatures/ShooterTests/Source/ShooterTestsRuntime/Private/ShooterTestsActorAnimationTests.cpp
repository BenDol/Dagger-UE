...
#include "Equipment/DaggerEquipmentManagerComponent.h"
#include "Equipment/DaggerPickupDefinition.h"
#include "Weapons/DaggerWeaponInstance.h"
#include "Weapons/DaggerWeaponSpawner.h"
...
{
	ADaggerWeaponSpawner* WeaponSpawnerPad{ nullptr };
	...
	{
		if (UDaggerEquipmentManagerComponent* EquipmentManager = Player->FindComponentByClass<UDaggerEquipmentManagerComponent>())
		{
			if (UDaggerWeaponInstance* WeaponInstance = EquipmentManager->GetFirstInstanceOfType<UDaggerWeaponInstance>())
			{
	...
	void SpawnWeaponSpawnerPad(const FString& WeaponDataAsset)
	{
		...
		UDaggerWeaponPickupDefinition* WeaponDefinition = Cast<UDaggerWeaponPickupDefinition>(WeaponData);
		ASSERT_THAT(IsNotNull(WeaponDefinition));
	
		WeaponSpawnerPad = &TObjectBuilder<ADaggerWeaponSpawner>(*Spawner, WeaponSpawnerPadBp)
