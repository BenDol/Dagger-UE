...
private:
	...
	void StartPie()
	{
		...
		SessionParams.GameModeOverride = ADaggerGameModeBase::StaticClass();
	...
	*/
	...
	{
		const ADaggerCharacter* RegisteredPlayer = NetworkState.LocalPlayer->GetLyraCharacter();

		TArray<AActor*> Pawns;
		UGameplayStatics::GetAllActorsOfClass(NetworkState.World, ADaggerCharacter::StaticClass(), Pawns);
		for (AActor* Pawn : Pawns)
		{
			ADaggerCharacter* Player = Cast<ADaggerCharacter>(Pawn);
			if (Player == nullptr)
			{
				continue;
			}
	...
	*/
	...
	/** Duration to allow for the Dagger loading screen. */
