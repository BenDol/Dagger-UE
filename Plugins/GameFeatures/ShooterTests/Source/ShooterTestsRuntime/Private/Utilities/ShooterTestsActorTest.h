...
#include "Experience/DaggerExperienceManagerComponent.h"
...
struct ShooterTestsActorBaseTest : public TTest<Derived, AsserterType>
{
	...
	bool HasWorldLoaded()
	{
		...
		UDaggerExperienceManagerComponent* ExperienceComponent = GameState->FindComponentByClass<UDaggerExperienceManagerComponent>();
	...
	/** Get our Dagger Player Pawn and all associated systems and functionality needed for our Player. */
	virtual void PreparePlayerPawn()
	{
		Player = Cast<ADaggerCharacter>(Spawner->FindFirstPlayerPawn());
		ASSERT_THAT(IsNotNull(Player));

		AbilitySystemComponent = Player->GetDaggerAbilitySystemComponent();
	...
	/** Setup the test by loading in the specified level and making sure that the Dagger player is fully spawned in before continuing. */
	...
	ADaggerCharacter* Player{ nullptr };
...
struct ShooterTestsActorAnimationTest : public ShooterTestsActorBaseTest<Derived, AsserterType>
{
	...
	/**
	 * Calls the parent method to get our Dagger Player Pawn and all associated systems and functionality needed for our Player before setting up functionality needed for input handling and animations.
	 * 
