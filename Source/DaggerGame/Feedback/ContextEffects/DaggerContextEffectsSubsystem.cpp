...
#include "Feedback/ContextEffects/DaggerContextEffectsSubsystem.h"
#include "Kismet/GameplayStatics.h"
#include "NiagaraFunctionLibrary.h"
#include "NiagaraSystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(DaggerContextEffectsSubsystem)
...
void UDaggerContextEffectsSubsystem::SpawnContextEffects(
	...
	, float AudioPitch)
{
	...
	if (TObjectPtr<UDaggerContextEffectsSet>* EffectsLibrariesSetPtr = ActiveActorEffectsMap.Find(SpawningActor))
	{
		// Validate the pointers from the Map Find
		if (UDaggerContextEffectsSet* EffectsLibraries = *EffectsLibrariesSetPtr)
		{
			...
			for (UDaggerContextEffectsLibrary* EffectLibrary : EffectsLibraries->DaggerContextEffectsLibraries)
			{
...
bool UDaggerContextEffectsSubsystem::GetContextFromSurfaceType(
	TEnumAsByte<EPhysicalSurface> PhysicalSurface, FGameplayTag& Context)
{
	// Get Project Settings
	if (const UDaggerContextEffectsSettings* ProjectSettings = GetDefault<UDaggerContextEffectsSettings>())
	{
...
void UDaggerContextEffectsSubsystem::LoadAndAddContextEffectsLibraries(AActor* OwningActor,
	TSet<TSoftObjectPtr<UDaggerContextEffectsLibrary>> ContextEffectsLibraries)
{
	...
	UDaggerContextEffectsSet* EffectsLibrariesSet = NewObject<UDaggerContextEffectsSet>(this);

	// Cycle through Libraries getting Soft Obj Refs
	for (const TSoftObjectPtr<UDaggerContextEffectsLibrary>& ContextEffectSoftObj : ContextEffectsLibraries)
	{
		// Load Library Assets from Soft Obj refs
		// TODO Support Async Loading of Asset Data
		if (UDaggerContextEffectsLibrary* EffectsLibrary = ContextEffectSoftObj.LoadSynchronous())
		{
			// Call load on valid Libraries
			EffectsLibrary->LoadEffects();

			// Add new library to Set
			EffectsLibrariesSet->DaggerContextEffectsLibraries.Add(EffectsLibrary);
		}
	}
...
void UDaggerContextEffectsSubsystem::UnloadAndRemoveContextEffectsLibraries(AActor* OwningActor)
{
