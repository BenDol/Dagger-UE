...
#include "AnimNotify_DaggerContextEffects.h"
#include "Feedback/ContextEffects/DaggerContextEffectsLibrary.h"
#include "DaggerContextEffectsInterface.h"
#include "Components/SkeletalMeshComponent.h"
#include "Engine/World.h"
#include "DaggerContextEffectsSubsystem.h"
#include "NiagaraFunctionLibrary.h"
#include "Kismet/GameplayStatics.h"
#include "NiagaraSystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(AnimNotify_DaggerContextEffects)



UAnimNotify_DaggerContextEffects::UAnimNotify_DaggerContextEffects()
{
}

void UAnimNotify_DaggerContextEffects::PostLoad()
{
	Super::PostLoad();
}

#if WITH_EDITOR
void UAnimNotify_DaggerContextEffects::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
}
#endif

FString UAnimNotify_DaggerContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_DaggerContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			TArray<UObject*> DaggerContextEffectImplementingObjects;

			// Determine if the Owning Actor is one of the Objects that implements the Context Effects Interface
			if (OwningActor->Implements<UDaggerContextEffectsInterface>())
			{
				// If so, add it to the Array
				DaggerContextEffectImplementingObjects.Add(OwningActor);
			}
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UDaggerContextEffectsInterface>())
					{
						DaggerContextEffectImplementingObjects.Add(Component);
					}
				}
			}

			// Cycle through all objects implementing the Context Effect Interface
			for (UObject* DaggerContextEffectImplementingObject : DaggerContextEffectImplementingObjects)
			{
				if (DaggerContextEffectImplementingObject)
				{
					// If the object is still valid, Execute the AnimMotionEffect Event on it, passing in relevant data
					IDaggerContextEffectsInterface::Execute_AnimMotionEffect(DaggerContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					if (PreviewProperties.bPreviewPhysicalSurfaceAsContext)
					{
						...
						if (const UDaggerContextEffectsSettings* DaggerContextEffectsSettings = GetDefault<UDaggerContextEffectsSettings>())
						{
							if (const FGameplayTag* SurfaceContextPtr = DaggerContextEffectsSettings->SurfaceTypeToContextMap.Find(PhysicalSurfaceType))
							{
					...
					{
						// Check if it is in fact a UDaggerContextEffectLibrary type
						if (UDaggerContextEffectsLibrary* EffectLibrary = Cast<UDaggerContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_DaggerContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_DaggerContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
	FDaggerContextEffectAnimNotifyVFXSettings VFXPropertiesIn, FDaggerContextEffectAnimNotifyAudioSettings AudioPropertiesIn,
	bool bAttachedIn, FName SocketNameIn, bool bPerformTraceIn, FDaggerContextEffectAnimNotifyTraceSettings TracePropertiesIn)
{
	Effect = EffectIn;
