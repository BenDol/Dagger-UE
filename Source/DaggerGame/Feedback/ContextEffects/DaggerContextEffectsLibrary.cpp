...
#include UE_INLINE_GENERATED_CPP_BY_NAME(DaggerContextEffectsLibrary)


void UDaggerContextEffectsLibrary::GetEffects(const FGameplayTag Effect, const FGameplayTagContainer Context, 
...
void UDaggerContextEffectsLibrary::LoadEffects()
{
...
EContextEffectsLibraryLoadState UDaggerContextEffectsLibrary::GetContextEffectsLibraryLoadState()
{
	// Return current Load State
	return EffectsLoadState;
}

void UDaggerContextEffectsLibrary::LoadEffectsInternal()
{
	// TODO Add Async Loading for Libraries

	// Copy data for async load
	TArray<FDaggerContextEffects> LocalContextEffects = ContextEffects;

	// Prepare Active Context Effects Array
	TArray<UDaggerActiveContextEffects*> ActiveContextEffectsArray;

	// Loop through Context Effects
	for (const FDaggerContextEffects& ContextEffect : LocalContextEffects)
	{
		// Make sure Tags are Valid
		if (ContextEffect.EffectTag.IsValid() && ContextEffect.Context.IsValid())
		{
			// Create new Active Context Effect
			UDaggerActiveContextEffects* NewActiveContextEffects = NewObject<UDaggerActiveContextEffects>(this);
	...
	this->DaggerContextEffectLibraryLoadingComplete(ActiveContextEffectsArray);
}

void UDaggerContextEffectsLibrary::DaggerContextEffectLibraryLoadingComplete(
	TArray<UDaggerActiveContextEffects*> DaggerActiveContextEffects)
{
	// Flag data as loaded
	...
	ActiveContextEffects.Append(DaggerActiveContextEffects);
}
