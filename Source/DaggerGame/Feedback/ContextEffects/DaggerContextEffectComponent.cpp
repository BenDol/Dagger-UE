...
#include "DaggerContextEffectComponent.h"

#include "Engine/World.h"
#include "DaggerContextEffectsSubsystem.h"
#include "PhysicalMaterials/PhysicalMaterial.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(DaggerContextEffectComponent)
...
UDaggerContextEffectComponent::UDaggerContextEffectComponent()
{
...
void UDaggerContextEffectComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	{
		if (UDaggerContextEffectsSubsystem* DaggerContextEffectsSubsystem = World->GetSubsystem<UDaggerContextEffectsSubsystem>())
		{
			DaggerContextEffectsSubsystem->LoadAndAddContextEffectsLibraries(GetOwner(), CurrentContextEffectsLibraries);
		}
	}
}

void UDaggerContextEffectComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	if (const UWorld* World = GetWorld())
	{
		if (UDaggerContextEffectsSubsystem* DaggerContextEffectsSubsystem = World->GetSubsystem<UDaggerContextEffectsSubsystem>())
		{
			DaggerContextEffectsSubsystem->UnloadAndRemoveContextEffectsLibraries(GetOwner());
		}
	}
...
void UDaggerContextEffectComponent::AnimMotionEffect_Implementation(const FName Bone, const FGameplayTag MotionEffect, USceneComponent* StaticMeshComponent,
	...
	FVector VFXScale, float AudioVolume, float AudioPitch)
{
	...
	if (const UWorld* World = GetWorld())
	{
		...
		if (UDaggerContextEffectsSubsystem* DaggerContextEffectsSubsystem = World->GetSubsystem<UDaggerContextEffectsSubsystem>())
		{
			...
			// Spawn effects
			DaggerContextEffectsSubsystem->SpawnContextEffects(GetOwner(), StaticMeshComponent, Bone, 
...
void UDaggerContextEffectComponent::UpdateEffectContexts(FGameplayTagContainer NewEffectContexts)
{
	// Reset and update
...
void UDaggerContextEffectComponent::UpdateLibraries(
	TSet<TSoftObjectPtr<UDaggerContextEffectsLibrary>> NewContextEffectsLibraries)
{
