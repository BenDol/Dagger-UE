...
#include "Private/AssetTypeActions_DaggerContextEffectsLibrary.h"
#include "ToolMenu.h"
#include "ToolMenus.h"
...
DEFINE_LOG_CATEGORY(LogDaggerEditor);
...
{
	return HasNoPlayWorld() && !GetDefault<UDaggerDeveloperSettings>()->CommonEditorMaps.IsEmpty();
}
...
static TSharedRef<SWidget> GetCommonMapsDropdown()
{
	...
	for (const FSoftObjectPath& Path : GetDefault<UDaggerDeveloperSettings>()->CommonEditorMaps)
	{
		if (!Path.IsValid())
		{
			continue;
 ...
 * FDaggerEditorModule
 */
class FDaggerEditorModule : public FDefaultGameModuleImpl
{
	typedef FDaggerEditorModule ThisClass;
	...
	virtual void StartupModule() override
	{
		...
		if (!IsRunningGame())
		{
			FModuleManager::Get().OnModulesChanged().AddRaw(this, &FDaggerEditorModule::ModulesChangedCallback);
		...
		// Register the Context Effects Library asset type actions.
		{
			...
			TSharedRef<FAssetTypeActions_DaggerContextEffectsLibrary> AssetAction = MakeShared<FAssetTypeActions_DaggerContextEffectsLibrary>();
			DaggerContextEffectsLibraryAssetAction = AssetAction;
	...
	{
		UDaggerExperienceManager* ExperienceManager = GEngine->GetEngineSubsystem<UDaggerExperienceManager>();
	...
	virtual void ShutdownModule() override
	{
		// Unregister the Context Effects Library asset type actions.
		{
			...
			TSharedPtr<IAssetTypeActions> AssetAction = DaggerContextEffectsLibraryAssetAction.Pin();
...
private:
	TWeakPtr<IAssetTypeActions> DaggerContextEffectsLibraryAssetAction;
	FDelegateHandle ToolMenusHandle;
};

IMPLEMENT_MODULE(FDaggerEditorModule, DaggerEditor);
